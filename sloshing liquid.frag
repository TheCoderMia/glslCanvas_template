...
void main() {
    ...
    // 使用 sdHexagram 来计算形状
    float noise_position = smoothstep(-0.424, -0.032, -uv.y - -0.884);
    float radius_noise = noise(vec3(2.364 * uv, u_time * 0.348)) * -0.848 * noise_position;
    float radius = 0.420 + radius_noise;
    float model_dist = abs(sdHexagram(uv, radius));

    // 動態呼吸
    float breathing = sin(2.0 * u_time * 0.5 * pi / 5.0) * 0.5 + 0.560;
